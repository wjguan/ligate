...
from keras.layers import Conv2D, Conv3D, ZeroPadding2D, ZeroPadding3D, Activation, Add, add, Lambda, Input, Cropping3D
...
class PixelCNN3D(object):
    ...
    def _build_layers(self, x):
        ...
        z = Activation('relu')(y) 
        z = Conv3D(self.nb_filters_h, 1, strides=(1,1,1), activation='relu', padding='valid', kernel_initializer='he_normal',name='res_conv_1')(y)
        z = self._masked_conv(z, self.filter_size, self.nb_filters_h, 1, 'B')
        z = Activation('relu')(z) 
        z = Conv3D(2*self.nb_filters_h, 1, strides=(1,1,1), padding='valid', kernel_initializer='he_normal',name='res_conv_3')(z)
        ...
        # FInish up 
        z = Activation('relu')(z)
        z = Conv3D(self.nb_filters_d, 1, strides=(1,1,1), activation='relu', padding='valid',kernel_initializer='he_normal', name='conv_4')(z)
        z = Conv3D(256, 1, strides=(1,1,1), activation='softmax', padding='valid',kernel_initializer='he_normal', name='conv_5')(z)
